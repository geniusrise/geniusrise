...
class OpenAIClassificationFineTuner(OpenAIFineTuner):
    """
    A bolt for fine-tuning OpenAI models for classification tasks.

    This bolt uses the OpenAI API to fine-tune a pre-trained model for classification.

    The dataset should be in the following format:
    - Each example is a dictionary with the following keys:
        - 'text': a string representing the text to be classified.
        - 'label': a string representing the class label of the text.
    """
    ...
    def load_dataset(self, dataset_path: str, **kwargs) -> Dataset | DatasetDict | Optional[Dataset]:
        ...
        The directory should contain one subdirectory for each class, and each subdirectory should contain text files
        where each file represents an example of that class. The name of the subdirectory is used as the class label.
    ...
    def prepare_fine_tuning_data(
        ...
        # For classification tasks, we need to convert the data into the format expected by OpenAI
        df["prompt"] = df["text"]
