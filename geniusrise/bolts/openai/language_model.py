...
class OpenAILanguageModelFineTuner(OpenAIFineTuner):
    """
    A bolt for fine-tuning OpenAI models for language modeling tasks.

    This bolt uses the OpenAI API to fine-tune a pre-trained model for language modeling.

    The dataset should be in the following format:
    - Each example is a dictionary with the following key:
        - 'text': a string representing a piece of text that the model should learn to generate.
    """
    ...
    def prepare_fine_tuning_data(
        ...
        # For language modeling tasks, we need to convert the data into the format expected by OpenAI
        df["prompt"] = df["text"]
