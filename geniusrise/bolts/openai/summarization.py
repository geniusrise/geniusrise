...
class OpenAISummarizationFineTuner(OpenAIFineTuner):
    ...
    def load_dataset(self, dataset_path: str, **kwargs) -> Dataset | DatasetDict | Optional[Dataset]:
        ...
        - Two text files named 'document.txt' and 'summary.txt'. The 'document.txt' file should contain
            the documents to be summarized, one per line. The 'summary.txt' file should contain the
            corresponding summaries, also one per line.
        ...
        try:
            ...
            if os.path.isfile(os.path.join(dataset_path, "dataset_info.json")):
                ...
                data = []
                with open(os.path.join(dataset_path, "document.txt"), "r") as f:
                    documents = f.read().strip().split("\n")
                with open(os.path.join(dataset_path, "summary.txt"), "r") as f:
    ...
    def prepare_fine_tuning_data(
        ...
        - The 'prompt' field is created from the 'document' field.
        - The 'completion' field is created from the 'summary' field.
